...
░╚════╝░╚══════╝╚═╝░░░░░░╚═════╝░╚═════╝░╚══════╝╚═╝░░░░░╚═╝░╚════╝░╚═════╝░╚═════╝░
EOF
echo -e "\033[38;2;0;255;255mclaudemods Btrfs Installer From Squashfs v1.01 Build 02/05/2025\033[0m"
echo ""
...
while true; do
    ...
    if [[ -f "$img_xz_file" && "$img_xz_file" == *.img.xz ]]; then
        break
    fi
    echo -e "\033[1;31mError: File not found or not a .img.xz file\033[0m" >&2
done
read -p "Enter username: " username

# --- VALIDATE INPUTS ---
[[ -z "$drive" || -z "$img_xz_file" || -z "$username" ]] && { echo -e "\033[1;31mError: Missing inputs\033[0m" >&2; exit 1; }
[[ ! -e "$drive" ]] && { echo -e "\033[1;31mError: Drive $drive not found\033[0m" >&2; exit 1; }
[[ ! -f "$img_xz_file" ]] && { echo -e "\033[1;31mError: img.xz file missing\033[0m" >&2; exit 1; }
...
echo -e "\033[38;2;0;255;255m Extracting system image..."
sudo mkdir /mnt/temp
sudo mkdir /mnt/temp_mount
sudo bash img.xz.sh "$img_xz_file" || { echo "Image extraction failed"; exit 1; }

# Mount the temporary image
sudo mount -o loop /mnt/temp/temp.img /mnt/temp_mount || { echo "Temp image mount failed"; exit 1; }

# Copy contents from the temporary image to the root partition
sudo rsync -aHAxSr /mnt/temp_mount/ /mnt/ || { echo "File copy failed"; exit 1; }
